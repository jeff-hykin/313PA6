...
// defines 
    ...
    // Loop 
        ...
        #define For_            { int Max_Value = 
        #define _Vector         .size()     ; for (int LoopNumber=1; LoopNumber <= Max_Value ; LoopNumber++) { int EachItem = LoopNumber - 1;
        #define _String         .size()     ; for (int LoopNumber=1; LoopNumber <= Max_Value ; LoopNumber++) { int EachChar = LoopNumber - 1;
        #define EndFor           } }

    // Program
        #define StartProgram   int main(int argc, char const *argv[]) {  try { int NumberOfMainInputs = argc-1;  string ExecFileName  = argv[0];  vector<string> MainArguments;   { int Max_Value =  NumberOfMainInputs     ; for (int LoopNumber=1; LoopNumber <= Max_Value ; LoopNumber++) {    MainArguments.push_back(argv[LoopNumber]);    } }  ; srand (time(NULL));
        #define EndMain        } catch (Errors the_error) { cout <<  the_error  << '\n'; EndProgram();} }

    // template 
        #define Template template<class ANYTYPE> 

    // displaying info 
        // #define DontShowDecimals  cout << fixed << setprecision(0);
        // #define OnlyShow_         cout << setprecision(
        // #define _Digits                                  );
        // #define OnlyShow_             cout << fixed << setprecision(
        // #define _Decimals                                        );
    ...
    // Equation solver 
        #define Solve_LessThanInequality_For_test_value { long double Step_ = 0.1; bool trues_Only = true ; bool falses_Only = true ; while ( Step_ > 0.0001 ) { if ( Step_ > 1000000000 ) { cout <<  "either the equation is unsolveable or the answer is > 1,000,000,000 "  << '\n'; ; EndProgram } bool Is_Undershooting = 
        #define EndLessThanSolve ; trues_Only = trues_Only && Is_Undershooting ; falses_Only = falses_Only && !Is_Undershooting ; if (trues_Only || falses_Only) {Step_*=2;} else {Step_/=2.0;} if (!Is_Undershooting){test_value-=Step_;} else {test_value+=Step_;}  } /* endwhile */ } /*}*/
            ///////////////////////
            //     // to use it, 
            //     // you must declare the test_value 
            //     // which can be a float, double, or long double
            //     // set the test_value equal to a number that 
            //     // you think will be close to the right answer 
            //     // then type Solve_LessThanInequality_For_test_value
            //     // then type your equation (on the same line) 
            //     // then type EndLessThanSolve
            // here's what the code actually looks like 
                // { // create a local space so vars don't interfere 
                // long double Step_ = 0.1 ;
                // bool trues_Only  = true ;
                // bool falses_Only = true ;
                // while ( Step_ > 0.0001 ) { 
                // if ( Step_ > 1000000000 ) {
                //   cout << ( "either the equation is unsolveable or this can't find the answer or both" ) ; EndProgram }
                // bool Is_Undershooting = WHATEVER_<_INEQUALITY_YOU_WANT  ;
                // trues_Only  = trues_Only  and  Is_Undershooting  ;
                // falses_Only = falses_Only and !Is_Undershooting  ;
                // if ( trues_Only or falses_Only ) 
                //     // this accelerates it if the initial guess is 
                //     // far away from the answer
                //     Step_ = Step_ * 2 ; 
                // else 
                //     Step_ = Step_ / 2.0 ;
                // 
                // // this just goes forward if undershooting 
                // // and goes backwards if overshooting 
                // if ! ( Is_Undershooting )
                //   test_value = test_value - Step_ ;
                // else 
                //   test_value = test_value + Step_ ;
                // } // endwhile 
                // } // end local 
