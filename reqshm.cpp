...
// Constructors
// 
    SharedMemory::SharedMemory(const string input_name, const RequestChannel::Side input_side) : name(input_name), side(input_side), side_name((input_side == SERVER_SIDE) ? "SERVER" : "CLIENT")
        {
            // Summary:
...
// Getters
// 
    string SharedMemory::get_name                  () { return name; }
    int    SharedMemory::get_read_file_descriptor  () { return read_file_descriptor; }
    int    SharedMemory::get_write_file_descriptor () { return write_file_descriptor; }
// 
// Methods
// 
    string SharedMemory::pipe_name       (RequestChannel::Mode mode)
        {
    ...
    void   SharedMemory::create_pipe     (string pipe_name_argument)
        {
            mkfifo(pipe_name_argument.c_str(), 0600) < 0;
        }

    void   SharedMemory::open_write_pipe (string pipe_name_argument)
        {
    ...
    void   SharedMemory::open_read_pipe  (string pipe_name_argument)
        {
            create_pipe(pipe_name_argument);
    ...
    string SharedMemory::cread           ()
        {
            ...
            if(read(read_file_descriptor, buf, MAX_MESSAGE) <= 0)
                {
                    cerr << "Error in SharedMemory cread()" << "\n";
                }
            return buf;
        }
